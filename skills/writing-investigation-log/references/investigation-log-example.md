# cli/cli PR #13740 と Shakespeare public dataset クエリ結果の確認

## 問い

- `cli/cli` の PR #13740 が、`actions/setup-go` を `v6.4.0` から `v6.5.0` に更新するPRか確認する。
- `bigquery-public-data.samples.shakespeare` で、`hamlet` の単語出現数上位5件を確認する。

## 現時点の結論

- `cli/cli` の公開PRでは、`actions/setup-go` を `v6.4.0` から `v6.5.0` に更新していることを、PR本文と差分から確認できる。
- dry run では、対象クエリが 5,114,816 bytes を処理する見込みであることを確認できる。
- 実クエリでは、`hamlet` における `word_count` 上位5件が `the`, `and`, `to`, `of`, `I` であることを確認できる。
