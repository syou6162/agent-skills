...
compatibility: Requires copilot CLI or codex CLI, and esa-llm-scoped-guard (optional, for dev diary)
---
...
```bash
cat .claude_work/review_prompt.md | codex review -
```
...
- すべての指摘はタスクリストに入れること（忘れ防止）
- **同じ指摘が既にタスクリストにある場合は、新規作成せずタスクを更新する**（重複防止）
