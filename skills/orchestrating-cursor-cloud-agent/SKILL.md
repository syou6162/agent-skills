...
### パターンD: オーケストレーター自身がレビュアー

1. `gh pr checkout <pr-url>` 等で該当PRのブランチを手元にチェックアウトする
...
オーケストレーターレビュー（パターンD）と Copilot レビュー対応（パターンC）を同じ Pull Request で行う場合：

1. まずオーケストレーター自身がレビューして修正指示 → CI green 確認
...
この順序にする理由：オーケストレーターレビューで構造的な問題を先に直しておくと、Copilot の指摘の一部が自然に解消される場合がある。
...
- **オーケストレーター自身がコードの修正やPull Requestの変更を直接行わないこと**。Cursorが後追いで修正を行い、変更が競合してぐちゃぐちゃになるため。修正が必要な場合は必ず `cursor-agent-cli run` 経由でCursorに指示すること。レビューで問題を発見した場合も、自分で直すのではなくCursorへの修正プロンプトとして投入する。
