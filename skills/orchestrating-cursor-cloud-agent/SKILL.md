...
Pull Requestのコードレビューは、**PRブランチを手元にチェックアウトしてから**行います。`gh pr diff` だけでは差分テキストしか見えず、呼び出し元・関連ファイル・既存の設計との整合性まで踏み込んだレビューができません。
...
- **レビュー前に必ず `gh pr checkout` でPRブランチを手元に持ってくること**。差分の確認だけを `gh pr diff` に任せてはいけない（局所的なレビューに留まる）。
...
- **`gh pr diff` だけでレビューを完結させないこと**。PRブランチを手元にチェックアウトし、関連ファイルも読んで文脈を踏まえたレビューを行う。
