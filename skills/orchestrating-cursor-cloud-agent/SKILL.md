...
**推奨: `gh pr diff` を使う（ローカルブランチの状態に依存しない）**
...
2. リモートの最新状態を取得する（ローカル `main` のチェックアウトや pull は不要）：

```bash
git fetch origin <デフォルトブランチ名>
```

3. 差分はローカルブランチではなく `origin/<デフォルトブランチ名>` を基準にする：
...
git diff origin/<デフォルトブランチ名>...pr-<PR番号>
```

<important>

- **差分の基準は `origin/<デフォルトブランチ名>` を使うこと**。ローカルの `<デフォルトブランチ名>` は古いことが多く、PRの実際の差分と一致しない。
- `git fetch origin` でリモート追跡ブランチを更新すれば十分。レビューのためにローカル `main` をチェックアウトして `git pull` する必要はない。
- 可能なら `gh pr diff` を優先する。GitHub上のPR差分と一致し、ローカルリポジトリの鮮度に左右されない。
...
- Pull Requestレビュー前に `git fetch origin` し、差分は `origin/<デフォルトブランチ名>` または `gh pr diff` で確認する（ローカル `main` は古くて実際のPR差分と一致しないことがある）
