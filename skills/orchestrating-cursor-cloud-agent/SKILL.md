...
Cursorへのタスクプロンプト（`create` / `run`）には、以下の指示を**毎回末尾に**含めること:

> コード変更を push した後、必ず `gh pr checks <PR番号> --repo <owner>/<repo>` で CI の状態を確認してください。
> 失敗しているチェックがあれば、`gh run view <run_id> --repo <owner>/<repo> --log-failed` でログを確認し、原因を特定して修正してください。
> すべてのチェックが pass するまで修正・push・確認を繰り返してください。
