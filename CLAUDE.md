...
- `skills/orchestrating-cursor-cloud-agent/SKILL.md` - Cursor Cloud Agentにタスクを委託し、SSEストリーム監視（`stream`）・キャンセル（`cancel`）・ポーリング（フォールバック）・レビュー・フォローアップを行うスキル。オーケストレーターはDevinに限らずCodex・Claude Code等を想定（自動発動）
