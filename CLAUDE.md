...
このリポジトリは、Claude Code用のスキル集です。実行可能コードではなく、Claude Codeに対する指示書（マークダウンファイル）を管理しています。Claude Codeプラグインシステムで管理され、プラグインとしてインストールされます。
...
### プラグイン設定
- `.claude-plugin/plugin.json` - プラグインマニフェスト（メタデータ定義）
...
### 公式ドキュメント参照
スキル、プラグインの実装や修正が必要になった場合は、**必ず該当する公式ドキュメントを参照すること**：
...
### 外部依存関係
スキルごとに以下のツールを使用：
- **GitHub CLI (`gh`)** - Pull Request操作（updating-pr-title-and-description）
