...
### orchestrating-cursor-cloud-agent
Cursor Cloud Agentにタスクを委託するよう依頼された際に自動的に発動するスキル。エージェント作成、SSEストリーム監視（`stream`）、実行キャンセル（`cancel`）、ステータスポーリング（フォールバック）、Pull Request状態確認、Linear紐付け、レビュー・フォローアップを行います。コーディング作業はCursorに任せ、オーケストレーター（Devin・Codex・Claude Code等）は判断・監視・指示に特化します。
