...
### devin-handoff-to-cursor
Devin CLIからDevin Cloudにハンドオフされた際に自動的に発動するスキル。受け取ったタスクを自分で実装せず、orchestrating-cursor-cloud-agentスキルの手順に従ってCursor Cloud Agentにオーケストレーションとして委託します。ハンドオフコンテキスト（リポジトリ、ブランチ、未コミットの変更、会話コンテキスト）を整理してCursorへのプロンプトを構築し、監視・レビュー・フォローアップを行います。
