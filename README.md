...
### writing-investigation-log
調査・裏取り・検証を行う際に自動的に発動するスキル。清書用ドキュメントとは分離して、`.agent_work/investigations/` にエージェント向けの内部調査ログを残します。根拠URL、ローカルパス、実行コマンド、BigQueryなどのクエリ、結果、未確認事項をテンプレートに沿って記録し、同じ調査の繰り返しや根拠不明の記述を防ぎます。
