...
### execute-plan
planモードの計画に基づいて実装を開始するスキル（ユーザーが明示的に呼び出す形式）。planファイルを読み取り、タスクリストを自動作成し、各タスクを順次実行してタスクごとにコミットを作成します。最後にcodex-reviewを実行して品質を確保します。ユーザーが `/syou6162-plugin:execute-plan` で明示的に呼び出す設計になっています。
