...
│   │   ├── SKILL.md                         # スキル定義
│   │   └── references/
│   │       ├── implementer.md               # 実装担当の詳細指示
│   │       ├── release-manager.md            # リリース担当の詳細指示
│   │       ├── reviewer.md                  # レビュー担当の詳細指示
│   │       └── plan-updater.md              # プラン更新担当の詳細指示
│   ├── codex-review/
│   │   └── SKILL.md                         # スキル定義
│   ├── codex-plan-review/
│   │   └── SKILL.md                         # スキル定義
...
### assembling-dev-team
「開発チーム集合」「チームで実装」「チーム編成して」の言及時に自動的に発動するスキル。プランファイルに基づいて実装・リリース・レビュー・プラン更新の4担当をAgent Teamsでスポーンし、ステップごとに実装→レビュー→コミットのサイクルを回します。リーダー（メインエージェント）がタスク管理・全体調整を行い、各メンバーが専門の役割を担当します。プラン更新担当はユーザーからの指摘やフィードバックをプランファイルに反映し、プランファイルをシングルソースオブトゥルースとして維持します。
...
### execute-plan
planモードの計画に基づいて実装を開始するスキル（ユーザーが明示的に呼び出す形式）。planファイルを読み取り、タスクリストを自動作成し、各タスクを順次実行してタスクごとにコミットを作成します。最後にcodex-reviewを実行して品質を確保します。agent teamモードでは別のワークフロー（assembling-dev-team）があるため、ユーザーが `/syou6162-plugin:execute-plan` で明示的に呼び出す設計になっています。
