...
### requesting-cursor-delegation
Devin CLIから `/handoff` でDevin Cloudにタスクを引き継ぐ際に発動するスキル。ハンドオフ先のDevin Cloudに対して、タスクの実行をCursor Cloud Agentに委譲するよう指示を含めます。委譲の流れは Devin CLI → Devin Cloud → Cursor Cloud Agent の二段階です。
...
### writing-investigation-log
調査・裏取り・検証を行う際に自動的に発動するスキル。清書用ドキュメントとは分離して、`.agent_work/investigations/` にエージェント向けの内部調査ログを残します。根拠URL、ローカルパス、実行コマンド、BigQueryなどのクエリ、結果、未確認事項をテンプレートに沿って日本語で記録し、同じ調査の繰り返しや根拠不明の記述を防ぎます。
