...
Yasuhisa Yoshida's personal agent skills, commands, and sub-agents for Claude Code.

## Overview

このリポジトリは、Claude Code用のagent skills（スキル、コマンド、サブエージェント）を管理する個人用リポジトリです。Claude Codeのプラグインシステムを使用してインストール・管理できます。
...
```bash
/syou6162-agent-skills:command-name argument
```
...
### execute-plan
planモードの計画に基づいて実装を開始するスキル（ユーザーが明示的に呼び出す形式）。planファイルを読み取り、タスクリストを自動作成し、各タスクを順次実行してタスクごとにコミットを作成します。最後にcodex-reviewを実行して品質を確保します。ユーザーが `/syou6162-agent-skills:execute-plan` で明示的に呼び出す設計になっています。
...
```bash
# 使用方法 (Claude Code内で)
/syou6162-agent-skills:triage_pr_comments https://github.com/owner/repo/pull/123
```
...
```bash
# 使用方法 (Claude Code内で)
/syou6162-agent-skills:estimate_pr_size
```
...
```bash
# 使用方法 (Claude Code内で)
/syou6162-agent-skills:optimize_bq_query query.sql
```
...
```bash
# 使用方法 (Claude Code内で)
/syou6162-agent-skills:multi_perspective_review
```
